...
- **Multi-tenant (MTX)**: CAP's Service Manager automatically routes each tenant to its own HDI container. Metrics and `Caches` rows are written only inside a tenant context (`cds.spawn` for the persistence timer). When metrics or the Caching API are enabled, `cds build` also emits `Caches` / `Metrics` / `KeyMetrics` `.hdbtable` files and matching `CachingApiService` `.hdbview` projections (same pattern as `CacheStore`), so tenant HDI deploys include those objects even if the cache store is Redis rather than `cds`. See [MTX Hybrid Test](mtx-hybrid-test.md).
