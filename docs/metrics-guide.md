...
- On HANA, enable metrics (or the Caching API) so `cds build` emits `Caches` / `Metrics` / `KeyMetrics` `.hdbtable` artifacts and `CachingApiService` `.hdbview` projections into the tenant deploy — required even if the cache `store` is Redis or memory.
