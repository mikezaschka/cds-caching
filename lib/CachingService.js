...
class CachingService extends cds.Service {
    ...
    async loadRuntimeConfiguration() {
        try {
            ...
            if (cacheConfig) {
                this.statistics.enableKeyTracking(cacheConfig.enableKeyTracking || false);
                this.statistics.enableStatistics(cacheConfig.enableStatistics || false);
            } else {
    ...
    async setMetricsEnabled(enabled) {
        ...
        this.log.debug(`Statistics ${enabled ? 'enabled' : 'disabled'} for cache ${this.name}`);
    }
    ...
    async setKeyMetricsEnabled(enabled) {
        ...
        this.log.debug(`Key tracking ${enabled ? 'enabled' : 'disabled'} for cache ${this.name}`);
    }
     ...
     * 
     * @example
     * const cache = await cds.connect.to('cache');
     * const response = await cache.send(req, service, { ttl: 60 });
     * 
     * @see: https://cap.cloud.sap/docs/node.js/core-services#srv-send-request
    ...
    async send() {
        ...
        // Not our signature? Not our problem
    ...
    extractMetadataFromRequest(req) {
        ...
        if (req.query) {
            dataType = 'request';
            operation = req.query.SELECT ? 'SELECT' : 'READ';
            serviceName = req.target?._service?.name;
            entityName = req.target?.name || '';

        } else {
            dataType = 'function';
            serviceName = req.target?.name || req.service?.name || req.target?._service?.name;
            operation = req.event;
        }
     ...
     * 
     * const cache = await cds.connect.to('cache');
     * const response = await cache.run(query, service, { ttl: 60 });
     *   
    ...
    wrap(key, asyncFunction, options = {}) {
        ...
        return async (...args) => {
            ...
            const metadata = {
                dataType: 'function',
                operation: 'WRAP',
                metadata: JSON.stringify({ functionName: asyncFunction.name || 'anonymous', args: args })
            };
    ...
    async set(key, value, options = {}) {
        ...
        const createdKey = this.createKey(key, options.key);
        await this.send('SET', {
            key: createdKey,
            value: wrappedValue,
            ttl: options.ttl || 0
        ...
        // Record the native set operation
        if (this.statistics?.options.enabled) {
            const metadata = {
                dataType: 'operation',
                operation: 'SET',
        ...
        try {
            ...
            if (this.statistics?.options.enabled) {
                const metadata = {
                    dataType: 'operation',
                    operation: 'GET',
                    metadata: JSON.stringify({ key: key })
                };
                const isHit = wrappedValue !== undefined && wrappedValue !== null;
                this.statistics.recordNativeGet(key, isHit, metadata);
            }
            
            return wrappedValue?.value;
        } catch (error) {
            if (this.statistics?.options.enabled) {
                this.statistics.recordNativeError(error);
            }
            throw error;
        }
    }
    ...
    async delete(key) {
        ...
        await this.send('DELETE', { key: createdKey });
    ...
    async clear() {
        ...
        if (this.statistics?.options.enabled) {
            const metadata = {
                dataType: 'operation',
                operation: 'CLEAR',
                metadata: JSON.stringify({ cache: this.name })
            };
    ...
    async deleteByTag(tag) {
        ...
        if (this.statistics?.options.enabled) {
            const metadata = {
                ...
                metadata: JSON.stringify({ tag: tag, cache: this.name })
            };
