...
const { normalizeCachingConfig, getStatisticsHandlerOptions } = require('./config-normalizer');
...
class CachingService extends cds.Service {
    ...
    async init() {
        ...
        if (normalized.metrics?.enabled === true && isPluginModelAvailable()) {
            try {
                await this.runtimeConfigManager.setMetricsEnabled(true);
            } catch (error) {
                this.log.warn(`Failed to persist metrics.enabled from config for ${this.name}:`, error);
            }
        }
        if (normalized.metrics?.keyMetricsEnabled === true && isPluginModelAvailable()) {
            try {
                await this.runtimeConfigManager.setKeyMetricsEnabled(true);
            } catch (error) {
