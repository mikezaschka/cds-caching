...
function normalizeCachingConfig(raw = {}) {
    ...
    if (raw.statistics) {
        const msg = warnOnce(
            'statistics',
            'cds-caching: "statistics" is deprecated; use "metrics" instead. See docs/migration-guide.md.'
        )
    ...
    if (raw.dashboard === true) {
        const msg = warnOnce(
            'dashboard',
            'cds-caching: "dashboard": true is deprecated; use metrics.reuse.dashboard instead. See docs/migration-guide.md.'
        )
        if (msg) legacyWarnings.push(msg)
