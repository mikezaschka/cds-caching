...
class CapOperations {
    ...
    async safeCacheOperation(operation, operationName, context = {}) {
        try {
            ...
            this.log._debug && this.log.debug('RESULT', { result, operationName, context });
    ...
    async _send(request, service, options) {
        ...
        // Mutating HTTP verbs are never cached. The one exception is HCQL, which
        // sends read `SELECT` queries as an HTTP POST body (CAP auto-selects HCQL
        // for CAP-to-CAP remote consumption). A verb-only check would silently
        // skip caching for those reads, so a POST that carries a SELECT query is
        // treated as a cacheable read. Non-HTTP read-throughs (e.g. function
        // sends with no HTTP method) remain cacheable as before.
        const method = request.method?.toUpperCase();
        const isMutatingMethod = ['POST', 'PUT', 'DELETE', 'PATCH'].includes(method);
        const isHcqlRead = method === 'POST' && !!request.query?.SELECT;
        if ((isMutatingMethod && !isHcqlRead) || !service.send) {
                ...
                case "cds.ql":
                    ...
                    if (query.SELECT) {
                        ...
                        try {
                            ...
                            const wrappedValue = {
                                value: data,
                                tags: this.tagResolver.resolveTags(options.tags, data, { ...query.params, hash: this.keyManager.createContentHash(query) }),
                                timestamp: Date.now()
                            };
