...
class CapOperations {
    ...
    constructor(cache, keyManager, statistics, log) {
        this.cache = cache;
    ...
    async send(request, service, options) {
        ...
        if (await this.cache.has(key)) {
            ...
            const wrappedValue = {
                value: response,
                tags: this.tagResolver.resolveTags(requestOptions.tags, response, { ...request.params, user: request.user?.id, tenant: request.tenant, locale: request.locale, hash: this.keyManager.createKey(request, keyParts, { template: '{hash}' }) }),
                timestamp: Date.now()
            };
            ...
            switch (arg1.constructor.name) {
                ...
                case "NoaRequest":
                    ...
                    if (hasCachedValue) {
                        ...
                        const wrappedValue = {
                            value: response,
                            tags: this.tagResolver.resolveTags(req.cacheOptions.tags, response, { ...req.params, hash: this.keyManager.createKey(req, { template: '{hash}' }) }),
                            timestamp: Date.now()
                        };
