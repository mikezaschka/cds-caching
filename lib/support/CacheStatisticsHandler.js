...
const {
    ...
    currentTenant,
    DEFAULT_TENANT,
} = require('./MultitenancyDetector');

/**
 * Class to handle all things caching statistics.
 *
 * In MTX mode, in-memory counters are partitioned by tenant. Persistence runs
 * via cds.spawn({ tenant }) so CAP routes writes to that tenant's HDI container.
 */
class CacheStatisticsHandler {
    ...
    _createEmptyBucket() {
        return {
            current: this._createEmptyCurrent(),
            historical: {
                hourly: new Map(),
                daily: new Map()
            },
    ...
    /**
     * Bucket key for the active context. Null means MTX with no tenant — do not record.
     * @returns {string|null}
     */
    _bucketKey() {
        if (!isMultitenantMode()) return DEFAULT_TENANT;
        if (!hasTenantContext()) return null;
        return currentTenant();
    }

    /**
     * Current tenant's stats bucket. Existing call sites use `this.stats.current`.
     */
    get stats() {
        const key = this._bucketKey();
        if (key === null) {
            if (!this._discardBucket) this._discardBucket = this._createEmptyBucket();
            return this._discardBucket;
        }
        if (!this._buckets.has(key)) {
            this._buckets.set(key, this._createEmptyBucket());
        }
        return this._buckets.get(key);
    }

    _canRecord() {
        return !(isMultitenantMode() && !hasTenantContext());
    }
    ...
    _metricsOn(tenantId) {
        ...
        if (key && this._overlays.has(key)) {
            ...
            if (overlay.metricsEnabled !== undefined) return overlay.metricsEnabled;
        }
        return this.options.metricsEnabled === true;
    }

    _keyMetricsOn(tenantId) {
        const key = tenantId ?? this._bucketKey();
        if (key && this._overlays.has(key)) {
            const overlay = this._overlays.get(key);
            if (overlay.keyMetricsEnabled !== undefined) return overlay.keyMetricsEnabled;
        }
    ...
    setupPersistenceInterval() {
        ...
        this.persistInterval = setInterval(async () => {
            try {
                ...
                this.log.error(`Persistence interval failed for cache ${this.options.cache}:`, error);
            }
    ...
    enableKeyMetrics(enabled) {
        if (isMultitenantMode() && hasTenantContext()) {
            ...
            overlay.keyMetricsEnabled = enabled;
    ...
    enableMetrics(enabled) {
        if (isMultitenantMode() && hasTenantContext()) {
            const t = currentTenant();
            const overlay = this._overlays.get(t) || {};
            overlay.metricsEnabled = enabled;
            this._overlays.set(t, overlay);
        } else {
    ...
    recordNativeSet(key, metadata = {}) {
        ...
        this.log.debug(`recordNativeSet called with key: ${key}, enabled: ${this._metricsOn()}, keyMetrics: ${this._keyMetricsOn()}`);
    ...
    recordNativeGet(key, hit, metadata = {}) {
        ...
        this.log.debug(`recordNativeGet called with key: ${key}, hit: ${hit}, enabled: ${this._metricsOn()}, keyMetrics: ${this._keyMetricsOn()}`);
    ...
    recordNativeDelete(key, metadata = {}) {
        ...
        this.log.debug(`recordNativeDelete called with key: ${key}, enabled: ${this._metricsOn()}, keyMetrics: ${this._keyMetricsOn()}`);
    ...
    recordNativeClear(metadata = {}) {
        ...
        // Log for debugging
        this.log.debug(`Recorded NATIVE CLEAR, enabled: ${this._metricsOn()}`);
    }
    ...
    recordNativeDeleteByTag(tag, metadata = {}) {
        ...
        // Log for debugging
        this.log.debug(`Recorded NATIVE DELETE BY TAG: ${tag}, enabled: ${this._metricsOn()}`);
    }
    ...
    /**
     * Reset current statistics for a tenant bucket.
     * @param {string} [tenantId] - tenant to reset; defaults to the active context bucket
     */
    resetCurrentStats(tenantId) {
        const key = tenantId ?? this._bucketKey();
        if (key === null) {
            this._discardBucket = this._createEmptyBucket();
            return;
        }
        // Drop the bucket so the map cannot grow without bound after persist
    ...
    async getAllTrackedKeys() {
        ...
        if (this._keyMetricsOn() && isPluginModelAvailable()) {
            try {
    ...
    /**
     * Persist statistics for every dirty tenant bucket (MTX).
     * Each tenant is persisted inside cds.spawn so CAP routes to that HDI.
     */
    async persistAllTenantMetrics() {
        const tenants = [...this._buckets.keys()];
        for (const tenant of tenants) {
            if (!this._metricsOn(tenant) && !this._keyMetricsOn(tenant)) continue;
            const bucket = this._buckets.get(tenant);
            if (!bucket || this._isBucketEmpty(bucket)) continue;

            await this._spawnPersist(tenant);
        }
    }
     ...
     * @private
     */
    _spawnPersist(tenant) {
        return new Promise((resolve) => {
            if (typeof cds.spawn !== 'function') {
                this.log.warn(`cds.spawn unavailable — cannot persist metrics for tenant ${tenant}`);
                resolve();
                return;
            }
            const job = cds.spawn({ tenant }, async () => {
                await this.persistMetrics();
            });
            job.on('succeeded', () => resolve());
            job.on('failed', (err) => {
                this.log.error(`Failed to persist metrics for tenant ${tenant}:`, err);
                resolve();
            });
        });
    }
    ...
    /**
     * Persist statistics for the current tenant context (or the single non-MTX bucket).
     */
    async persistMetrics() {
        ...
        if (isMultitenantMode() && !hasTenantContext()) {
            this.log.debug(`Skipping persistMetrics in MTX mode (no tenant context)`);
            return;
        }

        this.log.debug(`Starting persistStats for cache ${this.options.cache}, enabled: ${this._metricsOn()}`);
        ...
        const tenantKey = this._bucketKey() ?? currentTenant();
    ...
    async getMetrics(period = 'hourly', from, to) {
        if (!this._metricsOn() || !isPluginModelAvailable()) return null;
    ...
    async getKeyMetrics(key, from, to) {
        if (!this._keyMetricsOn() || !isPluginModelAvailable()) return null;
    ...
    async getCurrentKeyMetrics() {
        if (!this._keyMetricsOn()) return null;
    ...
    getPersistenceStatus() {
        return {
            ...
            tenantBuckets: this._buckets.size,
        };
    }
