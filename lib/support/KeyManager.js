        ...
        if (typeof keyOrObject === "object" && keyOrObject !== null) {
            switch (keyOrObject.constructor.name) {
                ...
                case "ODataRequest":
                    ...
                    // the reliable source for all OData query options. When the
                    // URL carries a query string it supersedes req.query.
                    //
                    // Protocols other than OData (HCQL, MCP) and programmatic
                    // CQL do NOT encode query options in the URL query string —
                    // they carry the differentiating query in req.query (the
                    // CQN). For HCQL the query travels in the HTTP POST body, so
                    // the URL is just the endpoint path (e.g. `/hcql/admin`),
                    // identical for every query. If we dropped req.query in that
                    // case, two different HCQL/MCP queries against the same
                    // endpoint would collide on the same cache key. Therefore we
                    // only skip req.query when the URL actually contains a query
                    // string; otherwise the CQN must contribute to the hash.
                    const url = this.normalizeUrl(keyOrObject._?.req?.url || keyOrObject.http?.req?.url);
                    const hasUrlQueryString = !!url && url.includes('?');
                    return this.createHash({
                        ...
                        query: hasUrlQueryString ? undefined : keyOrObject.query,
