...
const { normalizeCachingConfig } = require("./config-normalizer")
...
const createCacheEntry = async (cacheName, serviceConfig = {}) => {
    ...
    try {
        ...
        if (existingCache.length === 0) {
            // Leave metrics flags null so package.json remains the seed until an
            // operator explicitly overrides via set*Enabled.
            ...
            // Never clobber operator overrides — only refresh the mirrored config JSON.
