...
const { normalizeCachingConfig, metricsFlagsFromConfig } = require("./config-normalizer")
...
const getCachingServicesFromConfig = (srvs) => {
    ...
    for (const [serviceName, serviceConfig] of Object.entries(srvs)) {
        if (serviceConfig && serviceConfig.constructor && serviceConfig.constructor.name === "CachingService") {
            const requires = cds.env.requires?.[serviceName] || {};
            const normalized = normalizeCachingConfig(requires);

            cachingServices.push({
                name: serviceName,
                impl: serviceConfig.impl || requires.impl || 'cds-caching',
                store: serviceConfig.store || requires.store || 'memory',
                namespace: serviceConfig.namespace || requires.namespace || serviceName,
...
const createCacheEntry = async (cacheName, serviceConfig = {}) => {
    ...
    try {
        ...
        const requires = cds.env.requires?.[cacheName] || {};
        const flags = metricsFlagsFromConfig({ ...requires, metrics: serviceConfig.metrics ?? requires.metrics });
        ...
        if (existingCache.length === 0) {
            ...
            await db.update(Caches)
                .set({ config: JSON.stringify(serviceConfig), ...flags })
...
const scanCachingAnnotations = async (srvs) => {
    ...
    if (!isPluginModelAvailable()) {
        LOG.warn(
            ...
            `Enable metrics with optional metrics.reuse.api / metrics.reuse.dashboard, or wire manually via using … index.cds / cds add caching-metrics. ` +
